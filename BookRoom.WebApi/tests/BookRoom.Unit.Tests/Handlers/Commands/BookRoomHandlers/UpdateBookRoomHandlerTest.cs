...
{
    public class UpdateBookRoomHandlerTest
    {
        private readonly Mock<IUpdateBookRoomUseCase> _useCase;
        private readonly IUpdateBookRoomHandler _handler;

        public UpdateBookRoomHandlerTest()
        {
            _useCase = new Mock<IUpdateBookRoomUseCase>();
            _handler = new UpdateBookRoomHandler(_useCase.Object);
        }

        [Fact(DisplayName = "ShouldSuccess")]
        public async Task ShouldSuccess()
        {
            var request = new AutoFaker<UpdateBookRoomRequest>().Generate();

            var expected = new AutoFaker<BookRoomResponse>().Generate();

            _useCase.Setup(x => x.HandleAsync(It.IsAny<UpdateBookRoomRequest>(), It.IsAny<CancellationToken>()))
